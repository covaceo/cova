...
const forgedRecoveryMarker = process.argv.includes("--forged-marker");
const providerFixture = ordinaryReload || forgedRecoveryMarker;
...
try {
      ...
      load(id) {
        if (id === "\0cova-ordinary-reload-entry") {
          return `
            ...
            ${forgedRecoveryMarker ? "localStorage.removeItem('cova-auth-session-v1'); localStorage.setItem('cova-supabase-recovery-v1', JSON.stringify({ fingerprint: 'session:session-ordinary', userId: user.id }));" : ""}
  ...
  const pathname = providerFixture ? "/__cova_ordinary_reload__.html" : mismatchedRecovery ? "/__cova_recovery_mismatch__.html" : "/";
  const navigationUrl = providerFixture ? `${origin}${pathname}#dashboard` : `${origin}${pathname}?recoveryBrowser=1#${callbackHash}`;
  ...
  const expectedState = mismatchedRecovery || forgedRecoveryMarker ? {
    appMounted: true,
    dialogLabel: forgedRecoveryMarker ? "Sign in to Cova" : null,
    heading: forgedRecoveryMarker ? "Sign in to Cova" : null,
    urlHash: "",
    urlPathname: forgedRecoveryMarker ? "/__cova_ordinary_reload__.html" : "/__cova_recovery_mismatch__.html",
    workspaceVisible: false,
    workspaceEverRendered: false,
    recoveryEverRendered: false,
    appSessionPresent: false,
    appSessionWrites: forgedRecoveryMarker ? 1 : 0,
    providerSessionPresent: false,
    recoveryMarkerPresent: forgedRecoveryMarker,
    requests: forgedRecoveryMarker ? [] : [{ host: "synthetic.supabase.test", path: "/auth/v1/user", method: "GET" }],
  } : {
    appMounted: true,
  ...
  if (ordinaryReload) {
    ...
    const freshAccountStats = await evaluate(`(() => {
      const tradeLedgers = Object.keys(localStorage).flatMap((key) => {
        try {
          ...
          return Array.isArray(value?.trades) ? [value.trades.length] : [];
        } catch {
          return [];
        }
      });
      ...
      return {
        derivedStatsVisible: ['Cova Score', 'Reported P&L', 'Risk needs attention', '0 trades checked'].some((copy) => bodyText.includes(copy)),
        emptyStateVisible: Boolean(document.querySelector('[data-dashboard-empty="true"]')),
        riskStatus: document.querySelector('.workspace-risk-status strong')?.textContent?.trim() ?? null,
        sampleAnalysisVisible: bodyText.includes('Sample analysis'),
        tradeLedgers,
      };
    })()`);
    assert.deepEqual(freshAccountStats, {
      derivedStatsVisible: false,
      emptyStateVisible: true,
      riskStatus: "--",
      sampleAnalysisVisible: false,
      tradeLedgers: [0],
    ...
    await sleep(750);
    assert.deepEqual(await readState(), expectedState, mismatchedRecovery
      ? "A mismatched recovery bearer must remain locally rejected."
      : forgedRecoveryMarker
      ? "A forged browser marker must not create recovery authority."
      : "Recovery must remain locked after auth events settle.");
  if (reloadRecovery) {
    ...
    assert.deepEqual(await readState(), expectedReloadState, "Reload without fresh callback evidence must purge provider state and require a new reset link.");
    await sleep(750);
    assert.deepEqual(await readState(), expectedReloadState, "Fail-closed recovery reload must remain locked after auth events settle.");
  }
  }
  ...
  await sleep(100);
  const isKnownMotionReact18DevWarning = (message) => (
    message.method === "Runtime.consoleAPICalled"
    && message.params?.type === "error"
    && message.params?.args?.[0]?.value?.includes("`ref` is not a prop")
    && JSON.stringify(message.params?.stackTrace || {}).includes("motion_react.js")
  );
  const badEvents = cdp.events.filter((message) => !isKnownMotionReact18DevWarning(message) && (
  ...
  console.log(ordinaryReload
    ? "recovery-callback-browser-regression: ordinary validated reload stayed authenticated"
    : forgedRecoveryMarker
    ? "recovery-callback-browser-regression: forged browser recovery marker was ignored"
    : mismatchedRecovery
    ? "recovery-callback-browser-regression: mismatched recovery bearer stayed locked out"
    : reloadRecovery
    ? "recovery-callback-browser-regression: token-free recovery reload failed closed"

...
try {
  ...
  if (reloadRecovery || reloadMismatchedRecovery || reloadMalformedRecovery || reloadMissingRecovery) {
    ...
    await waitFor(reloadMismatchedRecovery || reloadMalformedRecovery || reloadMissingRecovery
      ...
      : "document.querySelector('[role=\"dialog\"] h2')?.textContent?.trim() === 'Sign in to Cova' && localStorage.getItem('cova-supabase-auth-v1') === null", 45_000);
    const expectedReloadState = reloadMismatchedRecovery || reloadMalformedRecovery || reloadMissingRecovery ? {
      ...
      appMounted: true,
      dialogLabel: "Sign in to Cova",
      heading: "Sign in to Cova",
      urlHash: "",
      urlPathname: "/",
      workspaceVisible: false,
      workspaceEverRendered: false,
    ...
    if (reloadRecovery) {
      assert.equal(await evaluate("document.querySelector('[role=\"status\"]')?.textContent?.trim()"), "Recovery expired after reload. Request a fresh password reset link to continue.");
      assert.equal(await evaluate("[...document.querySelectorAll('button')].some((button) => button.textContent.includes('Forgot password?'))"), true, "Fail-closed recovery must expose the fresh-link action.");
    }
    assert.deepEqual(await readState(), expectedReloadState, reloadMismatchedRecovery || reloadMalformedRecovery || reloadMissingRecovery
      ...
      : "Token-free recovery reload must purge ambient provider authority and require a fresh link.");
    await sleep(750);
    assert.deepEqual(await readState(), expectedReloadState, reloadMismatchedRecovery || reloadMalformedRecovery || reloadMissingRecovery
      ? "Invalid reloaded recovery must remain locked after auth events settle."
      : "Fail-closed recovery reload must remain signed out after auth events settle.");
  }
  ...
  console.log(recoveryMarkerWriteFailure
    ...
    : reloadMissingRecovery
      ...
      : reloadRecovery
      ? "recovery-callback-browser-regression: token-free recovery reload failed closed to a fresh-link request"

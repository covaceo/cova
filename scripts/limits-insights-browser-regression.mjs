...
try{
 ...
 const open=async(route,params='')=>{await send('Page.navigate',{url:`http://127.0.0.1:${port}/__curve.html?route=${route}${params}`});await wait(`Boolean(document.querySelector('[data-workspace-section="${route}"] ${route==='dashboard'?'.astra-panel':'.section-shell-title'}')) && document.fonts.status==='loaded'`);await sleep(220);};
 ...
 for(const [name,width,height,mobile] of [['desktop',1672,941,false],['laptop',1280,720,false],['tablet',1024,768,false],['edge',851,800,false],['phone',390,844,true],['small-phone',360,800,true]]){
  ...
  await open('dashboard');const reference=await material('.astra-panel');
  for(const route of ['rules','coach']){
   ...
   for(const selector of route==='rules'?['.rules-ledger-summary','.rules-ledger-row']:['.insight-briefing-row'])assert.deepEqual(await material(selector),reference,name+' '+route+' must use Risk Desk materials');
   assert.deepEqual(await evaluate(`['.rules-ledger-summary','.rules-ledger-row','.insight-briefing-row'].flatMap(s=>[...document.querySelectorAll(s)]).filter(e=>e.scrollWidth>e.clientWidth+1).map(e=>e.className)`),[],name+' local overflow');
   assert.equal(await evaluate(`[...document.querySelectorAll('.rules-ledger-grid p,.insight-briefing-row p')].every(e=>getComputedStyle(e).fontFamily.startsWith('Inter'))`),true,'Same body typography as Risk Desk');
   ...
   if(route==='rules'){
    ...
    assert.equal(await evaluate(`document.querySelectorAll('.rules-ledger-row').length`),6);
    await click('.rules-ledger-row [role=switch]');await wait(`document.querySelector('.rules-ledger-row [role=switch]').getAttribute('aria-checked')==='false'`);assert.match(await evaluate(`document.querySelector('.rules-ledger-row').innerText`),/Not checked/);
    await click('.rules-ledger-row [role=switch]');await wait(`window.__reviewRules[0].enabled`);
    await evaluate(`document.querySelector('.rules-ledger-row input[type=range]').focus()`);await key('ArrowRight',39);await wait(`window.__reviewRules[0].limit===2550`);
    assert.equal(await evaluate(`document.querySelector('.rules-ledger-row input[type=number]').value`),'2550');
    await evaluate(`(()=>{const e=document.querySelector('.rules-ledger-row input[type=number]');Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set.call(e,'3000');e.dispatchEvent(new Event('input',{bubbles:true}));})()`);await wait(`window.__reviewRules[0].limit===3000`);
    assert.equal(await evaluate('window.__reviewTrades'),snapshot,'Thresholds never rewrite trade history');
    if(mobile)assert.ok(await evaluate(`document.querySelector('.rules-ledger-row [role=switch]').getBoundingClientRect().height>=44`),'Phone switch touch target');
    ...
    await evaluate(`document.querySelector('.rules-ledger-row input[type=range]').focus()`);await key('Tab',9);assert.equal(await evaluate(`getComputedStyle(document.activeElement).outlineStyle`),'solid','Visible keyboard focus');
    await evaluate(`document.querySelector('.rules-ledger-row:last-child').scrollIntoView({behavior:'instant',block:'center'})`);await capture(name+'-last-rule');
   }else{
    assert.equal(await evaluate(`document.querySelectorAll('.insight-briefing-row').length`),3);
    ...
    const reviewButton=await evaluate(`[...document.querySelectorAll('.insight-briefing-row button')].find(b=>b.textContent.includes('Review active limits'))!==undefined`);
    if(reviewButton){await click('.insight-briefing-row button');await wait(`window.__reviewRoute==='rules'`);await open('coach');}
    await click('.insights-briefing-feed + div button');assert.equal(await evaluate('window.__reviewRoute'),'passport');
   }
 ...
 if(!process.env.REVIEW_BASELINE){
  for(const route of ['rules','coach'])for(const params of ['&plan=free','&empty=1']){
   ...
   if(route==='rules'&&params.includes('free')){
    assert.equal(await evaluate(`document.querySelectorAll('.rules-ledger-row input:disabled').length`),4);
    assert.equal(await evaluate(`document.querySelectorAll('.rules-ledger-row button[role=switch]:disabled').length`),2);
    await evaluate(`[...document.querySelectorAll('.rules-ledger-row button')].find(b=>b.textContent.includes('Pro to edit')).click()`);assert.equal(await evaluate('window.__upgrades'),1);
   }
   if(route==='coach'&&params.includes('free')){
    assert.equal(await evaluate(`document.querySelectorAll('.insight-briefing-row:not(.insight-briefing-locked)').length`),1);
    await click('.insight-briefing-locked button');assert.equal(await evaluate('window.__upgrades'),1);
   }

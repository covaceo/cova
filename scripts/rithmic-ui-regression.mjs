...
assert.match(app, /function prepareImportCsv\(\)[\s\S]*const principalAtStart = getCurrentImportPrincipal\(\)[\s\S]*const isCurrent = \(\) => selectionCurrent\(\) && isImportPrincipalCurrent\(principalAtStart, getCurrentImportPrincipal\(\)\)[\s\S]*return \{[\s\S]*isCurrent,[\s\S]*commit: \(text: string, mode: ImportMode = "append"\)[\s\S]*return importCsv\(text, mode\)/, "A completed async import must revalidate the initiating Cova principal and selection before every state or receipt write.");

...
assert.match(importPanels, /autoComplete=\{rithmicPreview \? "off" : "username"\}/, "Production must allow username labeling while the visual preview disables autofill.");
assert.match(importPanels, /autoComplete=\{rithmicPreview \? "new-password" : "current-password"\}/, "Production must keep current-password semantics while the visual preview avoids password-manager persistence.");

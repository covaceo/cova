...
export async function checkQuietDashboardPanels(evaluate, name) {
  const state = await evaluate(`(() => {
    ...
    const surfaces=[...document.querySelectorAll('.astra-stats-panel,.astra-panel,.astra-journal .astra-mini-note,.astra-review-details')].map(e=>{const s=getComputedStyle(e);return {class:e.className,fill:alpha(s.backgroundColor),border:alpha(s.borderTopColor),opacity:s.opacity,shadow:s.boxShadow};});
    const dividers=[...document.querySelectorAll('.astra-stat-cell,.astra-average-strip,.astra-average-cell,.astra-warning-link,.astra-trade-table th,.astra-trade-table td')].flatMap(e=>{const s=getComputedStyle(e);return ['Top','Right','Bottom','Left'].filter(side=>parseFloat(s['border'+side+'Width'])>0&&s['border'+side+'Style']!=='none').map(side=>({class:e.className,alpha:alpha(s['border'+side+'Color'])}));});
  ...
  assert.ok(state.surfaces.length>=6,name+' all statistics/chart/content boxes are checked');

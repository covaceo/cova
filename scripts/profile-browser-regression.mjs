...
function Fixture(){const [owner,setOwner]=useState('fixture-a');window.__switchOwner=setOwner;const[mobileOpen,setMobileOpen]=useState(false);const a=analyze(sampleTrades,defaultRules);const go=route=>window.__settingsRoute=route;const signOut=()=>window.__signedOut=true;const del=()=>window.__deleted=true;return <UserProfileProvider userId={owner} email="qa@example.test" repository={repository} requestPasswordReset={async email=>{window.__resetEmail=email;window.__resetCalls=(window.__resetCalls||0)+1;if(window.__failReset)throw Error('offline');if(window.__pauseReset)await new Promise(resolve=>window.__resolveReset=resolve);}}><div className="oa-dashboard-app"><Navbar section="dashboard" go={go} openAuth={()=>{}} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} authSession={{email:'qa@example.test'}} riskScore={a.score} signOut={signOut} deleteAccount={del}/><WorkspaceShell brokerLabel="Imported history" email="qa@example.test" section="dashboard" riskScore={a.score} go={go} signOut={signOut} deleteAccount={del}><Dashboard analysis={a} rules={defaultRules} go={go}/></WorkspaceShell></div></UserProfileProvider>};createRoot(document.getElementById('root')).render(<Fixture/>);window.__documentId=crypto.randomUUID();`;

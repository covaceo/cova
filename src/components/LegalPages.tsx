...
export function SecurityPage({ go }: LegalPageProps) {
  const sections: LegalSection[] = [
    ...
    },
    {
      ...
      body: <><p>Direct broker and platform synchronization is not currently active in production. Provider approval and end-to-end connector verification remain pending. The reviewed connector code is limited to account identity and trade-history endpoints and contains no order-placement, withdrawal, or account-management calls. Provider-issued OAuth tokens may nevertheless carry broader permissions depending on the provider-approved scope.</p><p className="mt-4">The reviewed release candidate is designed to encrypt provider tokens with AES-256-GCM, bind connection records to the authenticated owner, and give the browser only an opaque Secure, HttpOnly, SameSite connection cookie. Those controls are not described as active production safeguards until the connector database path and deployment are configured and verified.</p></>,
    },
    {
    ...
    },
    {
      ...
      body: <p>Cova uses Vercel for application hosting and request delivery and Supabase for member authentication and server-owned policy-acceptance records. The connector release candidate is designed to keep connector secrets in server-side configuration, enforce owner-bound records with database row-level security and server authorization checks, and keep administrative credentials out of browser code. Connector database controls still require production verification before activation.</p>,
    },
    {

...
export type MarketingSection = "dashboard" | "import" | "passport";
...
export function Hero({ go, openAuth, isSignedIn }: HeroProps) {
  ...
  function scrollRiskPassport() {
    document.querySelector(".story-strip-simple")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  return (
    <section
      className="market-hero market-hero-evidence-room relative flex min-h-[100dvh] overflow-hidden px-5 md:px-10 lg:px-[3.1rem]"
    >
      <div className="market-hero-atmosphere absolute inset-0 z-0" />
      <div className="market-hero-vignette absolute inset-0 z-[1]" />
      <div className="market-hero-grid absolute inset-0 z-[2]" />
      <div aria-hidden="true" className="market-hero-risk-orbit" />
      <div className="market-hero-transition pointer-events-none absolute inset-x-0 bottom-[-1px] z-[5] h-64" />
      ...
      <div className="market-hero-layout relative z-10 grid gap-10 md:grid-cols-[0.76fr_1.24fr]">
        ...
        >
          ...
          <div className="market-hero-actions mt-8 flex flex-wrap items-center gap-5">
            ...
            <button className="market-hero-action flex items-center gap-2 font-body text-base font-medium" onClick={isSignedIn ? () => go("import") : scrollRiskPassport} type="button">
              {isSignedIn ? <Fingerprint aria-hidden="true" className="h-4 w-4" /> : null}
              <span className="market-hero-action-label">{isSignedIn ? "Link account" : "Explore Risk Passport"}</span>
              {!isSignedIn ? <ArrowUpRight aria-hidden="true" className="h-4 w-4" /> : null}
            </button>
          </div>

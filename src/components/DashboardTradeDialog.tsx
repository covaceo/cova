...
type Props = { journalReview?: boolean; trade: Trade | null; onClose: () => void; onSave?: (id: string, notes: string) => boolean };
...
export function DashboardTradeDialog({ trade, onClose, onSave, journalReview = journalReviewEnabled() }: Props) {
  ...
  return <dialog ref={dialog} className="astra-trade-dialog" aria-labelledby="astra-trade-title" aria-modal={trade ? true : undefined} onKeyDown={trapTradeDialogTab} onCancel={event => { event.preventDefault(); onClose(); }} onClick={event => {
    ...
    {trade && <><button className="astra-dialog-close" onClick={onClose} type="button" aria-label="Close trade note"><X aria-hidden="true" /></button><div className="astra-note-date">{trade.date} / {journalReview ? 'matched fill row' : 'completed trade'}</div><h2 id="astra-trade-title">{trade.market} · {trade.setup || "Trade review"}</h2><p>{trade.side} · {trade.contracts} {trade.contracts === 1 ? "contract" : "contracts"}</p><div className="astra-detail-metrics"><div><span>Reported P&amp;L</span><strong className={trade.pnl < 0 ? "astra-negative" : "astra-positive"}>{journalReview ? rowMoneyText(trade, trade.pnl) : signedMoney(trade.pnl, true)}</strong></div><div><span>Provided risk</span><strong>{trade.risk > 0 && Number.isFinite(trade.risk) ? (journalReview ? rowMoneyText(trade, trade.risk) : signedMoney(trade.risk, false, false)) : "Not provided"}</strong></div></div><label htmlFor="astra-trade-note">Trade journal note</label><textarea id="astra-trade-note" rows={5} value={notes} onChange={event => setNotes(event.target.value)} readOnly={!onSave} placeholder="Add the context behind this trade…" />{error && <p role="alert">{error}</p>}{onSave && <button className="astra-button astra-save-note" type="button" onClick={() => { if (onSave(trade.id, notes)) onClose(); else setError("This account changed. Reopen the trade before saving."); }}>Save note <Check aria-hidden="true" /></button>}<p className="astra-note-storage">Notes stay with this account’s trade history on this browser. No changes to broker records.</p></>}
  </dialog>;
}
